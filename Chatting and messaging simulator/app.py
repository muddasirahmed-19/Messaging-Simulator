...
class ChatApp:
    def __init__(self, root):
        ...
        builder = ChatSessionBuilder().session_id("default").participants(["Muddasir", "BOT"])
    ...
    def send_message(self):
        ...
        msg = self.factory.create("text", "You", "BOT", content, status="sent")
    ...
    def simulate_incoming(self):
        def worker():
            time.sleep(1)
            msg = self.factory.create("text", "BOT", "You", "Hello there!", status="delivered")
            self.engine.send_message(self.session.session_id, msg)
